...
class robot{
    ...
    protected function bot($method,$datas=[]){
   ...
   public function Send2id($chat_id, $Message)
    ...
    public function sendProduct($product_array)
    {
        $this->bot('sendPhoto',[
            'chat_id' => $this->UserInfo()['Chat_id'],
            'photo' => $product_array['productPhoto'],
            'caption' => $product_array['productCaption'],
            'reply_markup' => json_encode([
                'inline_keyboard' => [
                    [['text' => "مشاهده محصول در وبسایت ",'url' => $product_array['productUrl']]]
                ]])
        ]);
    }
    ...
    public function CheckItem($fieldName,$fieldValue)
    {
        $this->copen();
